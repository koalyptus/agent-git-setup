...
- **`gh` (GitHub CLI) is required for the bot GitHub-actor path.** Local
  ...
  a **Bot**, `--preflight` refuses unless `AGENT_GIT_ALLOW_HUMAN_ACTOR=1` is
  explicitly exported — the account owner's conscious consent, with no default and no
  silent fallback. An account-owner PAT in `GH_TOKEN` is the classic bug (every `gh`/API
  call lands under YOU); this check catches it. When `gh`/network is
  unavailable it degrades to a WARNING (cannot verify identity) rather than a
  hard block, so hermetic/offline runs still proceed — but the agent must still
  mint the bot token as the documented happy path in that case.
- The bot identity:
  ...
  - `GH_TOKEN` — **required in the agent flow** (the agent opens PRs / acts on GitHub as the bot). Mint and export it before any `gh`/API call and before `--preflight`. Not needed for the local commit author alone. `--preflight` verifies the token resolves to a **Bot** actor (`gh api user`); if it resolves to an account owner, set `AGENT_GIT_ALLOW_HUMAN_ACTOR=1` to proceed as the account owner by explicit consent, or — preferably — re-mint the bot token.
  - `AGENT_GIT_ALLOW_HUMAN_ACTOR` — *(optional, default unset)* export `1` ONLY to deliberately run `gh`/API as the account owner. This is the explicit consent gate for the bot-actor verification above. There is no default: an account-owner PAT passed as `GH_TOKEN` makes `--preflight` fail closed until this is set. Never set it as a workaround for a missing bot token — mint the bot token instead (`source <(scripts/mint-token.sh --shell)`).
